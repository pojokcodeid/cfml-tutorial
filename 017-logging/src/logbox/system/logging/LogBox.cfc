...
component accessors="true" {
	 ...
	 */
	variables.DEFAULT_CONFIG = "logbox.system.logging.config.DefaultConfig";
	...
	function configure( required config ){
		...
		if ( !isObject( arguments.config ) && isStruct( arguments.config ) ) {
			arguments.config = new logbox.system.logging.config.LogBoxConfig().loadDataDSL( arguments.config );
		}
