...
component {
	 ...
	 *
	 * @return logbox.system.core.dynamic.MixerUtil
	 */
	function getMixerUtil(){
		if ( isNull( variables.mixerUtil ) ) {
			variables.mixerUtil = new logbox.system.core.dynamic.MixerUtil();
		}
	...
	boolean function isFamilyType( required family, required target ){
		...
		switch ( arguments.family ) {
			case "handler": {
				familyPath = "logbox.system.EventHandler";
				break;
			}
			case "interceptor": {
				familyPath = "logbox.system.Interceptor";
				break;
			}
			default: {
	...
	/**
	 * @deprecated Refactor to use the Env Delegate: logbox.system.core.delegates.Env
	 */
	function getSystemSetting( required key, defaultValue ){
		return new logbox.system.core.delegates.Env().getSystemSetting( argumentCollection = arguments );
	}
	...
	function getSystemProperty( required key, defaultValue ){
		return new logbox.system.core.delegates.Env().getSystemProperty( argumentCollection = arguments );
	}
	...
	function getEnv( required key, defaultValue ){
		return new logbox.system.core.delegates.Env().getEnv( argumentCollection = arguments );
	}
