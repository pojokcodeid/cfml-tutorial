...
component accessors="true" {
	...
	Future function exceptionally( required target ){
		variables.native = variables.native.exceptionally(
			createDynamicProxy(
				new logbox.system.async.cbproxies.models.Function(
					arguments.target,
					variables.debug,
	...
	Future function run(
		...
		var jSupplier = createDynamicProxy(
			new logbox.system.async.cbproxies.models.Supplier(
				arguments.supplier,
				arguments.method,
				variables.debug,
				variables.loadAppContext
	...
	Future function handle( required action ){
		var biFunction = createDynamicProxy(
			new logbox.system.async.cbproxies.models.BiFunction(
				arguments.action,
				variables.debug,
				variables.loadAppContext
			),
	...
	Future function whenComplete( required action ){
		var biConsumer = createDynamicProxy(
			new logbox.system.async.cbproxies.models.BiConsumer(
				arguments.action,
				variables.debug,
				variables.loadAppContext
			),
	...
	Future function thenRun( required target ){
		var fConsumer = createDynamicProxy(
			new logbox.system.async.cbproxies.models.Consumer(
				arguments.target,
				variables.debug,
				variables.loadAppContext
			),
	...
	Future function thenCompose( required fn ){
		variables.native = variables.native.thenCompose(
			createDynamicProxy(
				new logbox.system.async.cbproxies.models.FutureFunction(
					arguments.fn,
					variables.debug,
					variables.loadAppContext
				),
