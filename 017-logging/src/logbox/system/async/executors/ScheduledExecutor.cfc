...
component extends="Executor" accessors="true" singleton {
	...
	ScheduledFuture function schedule(
		...
		// Return the results
		return new logbox.system.async.tasks.ScheduledFuture( jScheduledFuture );
	}
	...
	ScheduledTask function newTask(
		...
		return new logbox.system.async.tasks.ScheduledTask( argumentCollection = arguments );
	}
	...
	function buildJavaRunnable( required task, required method ){
		return createDynamicProxy(
			new logbox.system.async.cbproxies.models.Runnable(
