component {
    ...
    public any function register(content) {
        ...
        return {
            userId: insertUser.generatedKey,
            name: content.name,
            phone: content.phone,
    ...
    public boolean function activateStatus(uuid) {
        var data = queryExecute('SELECT * FROM user WHERE uuid=:uuid', {uuid: {value: uuid, sqltype: 'CF_SQL_VARCHAR'}});
    ...
    public struct function update(content = {}) {
        transaction action="begin" {
            ...
            var qUpdate = queryExecute(
                ...
                {
                    name: {value: content.name, sqltype: 'CF_SQL_VARCHAR'},
                    phone: structKeyExists(content, 'phone') ? {value: content.phone, sqltype: 'CF_SQL_VARCHAR'} : {
                        value: '',
                        sqltype: 'CF_SQL_VARCHAR'
                    },
                    address: structKeyExists(content, 'address') ? {value: content.address, sqltype: 'CF_SQL_VARCHAR'} : {
                        value: '',
                        sqltype: 'CF_SQL_VARCHAR'
                    },
